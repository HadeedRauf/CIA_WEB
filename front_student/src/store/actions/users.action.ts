...
export function addAdmin(user: IUser): any {
    return async (dispatch : any) => {
        try {
            ...
            const message = (e && e.response && e.response.data) || (e && e.message) || "Failed to grant admin";
...
export function removeAdmin(user: IUser): (dispatch: any) => Promise<any> {
    return async (dispatch : any) => {
        try {
            ...
            const message = (e && e.response && e.response.data) || (e && e.message) || "Failed to revoke admin";
            return dispatch(addNotification("Error", message));
        }
    }
}
export function getUsers(): any {
    ...
    return async (dispatch : any) => {
        try {
            ...
            const message = (e && e.response && e.response.data) || (e && e.message) || "Failed to load users";
            return dispatch(addNotification("Error", message));
        }
    }
}
